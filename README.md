...
После категоризации скрипт `generate_moc.py` создаёт навигационные хабы — [Maps of Content](https://notes.linkingyourthinking.com/Cards/MOCs+Overview).

**Что это:** MOC — это заметка-оглавление, которая собирает ссылки на связанные документы по теме. Вместо плоского списка файлов вы получаете структурированную карту знаний.

**Пример:** `_MOC/health.md` содержит ссылки на все заметки про здоровье, сгруппированные по подтемам (фитнес, питание, медицина).

**Зачем:** Когда у вас 1000+ заметок, искать нужное через поиск неудобно. MOC даёт точку входа — открыл хаб по теме и видишь всё связанное.

### Про Obsidian

Этот инструмент создан с прицелом на [Obsidian](https://obsidian.md/) — бесплатный редактор заметок с мощной навигацией по связям между документами.

**Почему Obsidian рекомендован:**
- MOC-хабы (`_MOC/`) работают как интерактивные карты — клик по ссылке открывает заметку
- Теги кликабельны и показывают все связанные заметки
- Graph View визуализирует связи между документами
- Всё хранится локально в обычных `.md` файлах

**Но можно и без него.** Результат — стандартные markdown-файлы с YAML-frontmatter. Открываются в любом редакторе: VS Code, Typora, iA Writer, Bear, или просто в Finder. Структура папок работает везде.

### Рекомендуемый способ запуска

**Лучший вариант — через [Claude Code](https://claude.com/claude-code) или другой AI-ассистент** (Cursor, Windsurf, Cline и т.д.).

Просто скажите ассистенту:
> «Склонируй https://github.com/aturilin/document-categoriser, установи зависимости, настрой API ключ и запусти категоризацию моих заметок из папки X»

Ассистент сам:
- Клонирует репозиторий
- Установит Python-зависимости
- Создаст `.env` с вашим API ключом
- Запустит процесс и покажет прогресс
- Поможет разобраться если что-то пойдёт не так

Это особенно удобно если вы не работаете с командной строкой каждый день.
